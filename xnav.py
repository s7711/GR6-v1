...
class XNav:
    def __init__(self, ip_address):
        ...
        threading.Thread(target=self.download_xnav_config).start()
    ...
    def user_command(self, message):
        ...
        if message == '!send-time':
            ...
            self.upload_xnav_config()
        else:
            sock.sendto(bytes(message+"\n", "utf-8"), (CFG['InsIp'],3001))

    def download_xnav_config(self):
        """ Download the configuration from the xNAV and store it in
...
class XNav:
    ...
    def download_xnav_config(self):
        ...
        for f in fs:
            ...
            f'static/xnav-config/{f}.txt'], capture_output=True )
            if cp.returncode == 0:
                logging.info(f"Downloaded config file {f}")
            else:
                logging.info(f"Cannot download config file {f}")

    def upload_xnav_config(self):
        """Upload edited config files to the xNAV, preserving a timestamped backup."""
        if sys.platform == 'win32':
            logging.info("Upload not supported on Windows.")
            return

        # (a) Create timestamped backup directory
        timestamp = datetime.datetime.now().strftime('%y%m%d_%H%M%S')
        backup_dir = f'static/xnav-config-backup/{timestamp}'
        os.makedirs(backup_dir, exist_ok=True)

        # (b) Copy and rename .txt files
        source_dir = 'static/xnav-config'
        for filename in os.listdir(source_dir):
            if filename.endswith('.txt'):
                original_path = os.path.join(source_dir, filename)
                new_name = filename[:-4]  # Strip '.txt'
        ...
        # (c) Upload via curl to xNAV
        for filename in os.listdir(backup_dir):
            local_path = os.path.join(backup_dir, filename)
            remote_url = f'ftp://{self.ip_address}/{filename}'

            cp = subprocess.run(['curl', '-T', local_path, remote_url], capture_output=True)
            if cp.returncode == 0:
                logging.info(f"Uploaded {filename} to xNAV")
            else:
                logging.warning(f"Failed to upload {filename}: {cp.stderr.decode().strip()}")
