...
OUTPUTLATENCY2SEC   = 1e-2                # Units of 0.01 s
...
class NcomRx(object):
    def __init__(self):
        ...
        # def myCalc( nrx )
        # and should add to nac, status, connection, etc.
        self.moreCalcs = []
        self.connection['moreCalcErrors'] = 0
    ...
    def decode(self,rxBytes, machineTime=None):
        ...
        for calc in self.moreCalcs:
            try:
                calc(self)
            except:
                self.connection['moreCalcErrors'] += 1
    ...
    def decodeStatus32(self,statusBytes):
        ...
        self._updateInnovation( 'InnZeroVelY', statusBytes[1:2] )
        self._updateInnovation( 'InnZeroVelZ', statusBytes[2:3] )
        self._updateInnovation( 'InnNoSlipH', statusBytes[3:4] )
        self._updateInnovation( 'InnHeadingH', statusBytes[4:5] )
        self._updateInnovation( 'InnWSpeed', statusBytes[5:6] )
        self._updateInnovation( 'InnZeroVelX', statusBytes[6:7] )
        self._updateInnovation( 'InnZeroVelX', statusBytes[7:8] )
    ...
    def decodeStatus49(self,statusBytes):
        # Omnistar status information        
        self.status['OmniFreq'] = int.from_bytes(statusBytes[0:2], byteorder = 'little', signed=False)
        if statusBytes[0:2] == b'\xFF\xFF': del self.status['OmniFreq']
        
        self.status['OmniSNR'] = statusBytes[2]
        if statusBytes[2] == 0xFF: del self.status['OmniSNR']
        
        self.status['OmniTrackTime'] = statusBytes[3]
        if statusBytes[3] == 0xFF: del self.status['OmniTrackTime']

        self.status['OmniStatus'] = statusBytes[4]
        if statusBytes[0] == 0xFF: del self.status['OmniStatus']

        self.status['OmniSerial'] = int.from_bytes(statusBytes[5:8], byteorder = 'little', signed=False)
        if statusBytes[5:8] == b'\xFF\xFF\xFF': del self.status['OmniSerial']
    ...
    def decodeStatus75(self,statusBytes):
        x = int.from_bytes(statusBytes[0:2], byteorder = 'little', signed=False)
        if x != 0xFFFF:
...
def RbnHPR(H,P=None,R=None):
    ...
    """
    # If P is None then assume H is tuple (H,P,R)
    if P is None:
        P = H[1]
        R = H[2]
        H = H[0]
...
def LLA2NED(Lat, Lon, Alt, RefFrameLat, RefFrameLon, RefFrameAlt):
    """ Finds the local co-ordinates give LLA. All angles in degrees
    """
    RefFrameLatR = RefFrameLat * DEG2RAD
    # RefFrameLonR = RefFrameLon * DEG2RAD   !! Not needed
    
    # Earth radii
    tmp = EARTH_ECCENTRICITY * math.sin(RefFrameLatR)
    tmp = 1.0 - tmp * tmp
    sqt = math.sqrt(tmp)
    rho_e = EARTH_EQUAT_RADIUS * (1.0 - EARTH_ECCENTRICITY * EARTH_ECCENTRICITY) / (sqt * tmp)
    rho_n = EARTH_EQUAT_RADIUS / sqt
    RadLat = rho_e + RefFrameAlt
    RadLon = rho_n + RefFrameAlt
    
    NED = {}    
    NED["LocalN"] = (Lat-RefFrameLat) * DEG2RAD * RadLat
    NED["LocalE"] = (Lon-RefFrameLon) * DEG2RAD * RadLon * math.cos(RefFrameLatR)
    NED["LocalD"] = RefFrameAlt - Alt
    NED["LocalZ"] = Alt - RefFrameAlt
    
    return NED


def NED2LLA( LocalN, LocalE, LocalD, RefFrameLat, RefFrameLon, RefFrameAlt):
    """ Finds the LLA from LocalN, LocalE and LocalD
    (Probably not very good near the poles or for large NED)
    """
    RefFrameLatR = RefFrameLat * DEG2RAD
    # RefFrameLonR = RefFrameLon * DEG2RAD   !! Not needed
    ...
    rho_e = EARTH_EQUAT_RADIUS * (1.0 - EARTH_ECCENTRICITY * EARTH_ECCENTRICITY) / (sqt * tmp)
    rho_n = EARTH_EQUAT_RADIUS / sqt
    RadLat = rho_e + RefFrameAlt # radius about east axis affect lat
    RadLon = rho_n + RefFrameAlt # radius about north axis affects lon

    LLA = {}
    LLA["Lat"] = RefFrameLat + LocalN/(RadLat)*RAD2DEG
    LLA["Lon"] = RefFrameLon + LocalE/(RadLon)*RAD2DEG/math.cos(RefFrameLatR)
    LLA["Alt"] = RefFrameAlt - LocalD
